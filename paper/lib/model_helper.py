...
class LlamaHelperBase:
    def __init__(self, pretrained_model):
        self.tokenizer = AutoTokenizer.from_pretrained(pretrained_model, padding_side='left')
    ...
    def generate_text_batched(self, prompts, max_length=100, max_tokens_per_batch=4096):
            ...
            if total_tokens > max_tokens_per_batch:
                ...
                first_half = self.generate_text_batched(prompts[:mid_idx], max_length, max_tokens_per_batch)
                second_half = self.generate_text_batched(prompts[mid_idx:], max_length, max_tokens_per_batch)
            ...
            tokens = torch.cat([torch.nn.functional.pad(tp, (0, max_token_length - tp.size(1))).to(self.device) for tp in tokenized_prompts])
...
class Llama27BHelper:
    def __init__(self, pretrained_model="meta-llama/Llama-2-7b-hf"):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(pretrained_model, use_auth_token=token,padding_side='left')
        self.model = AutoModelForCausalLM.from_pretrained(pretrained_model, use_auth_token=token)
        success = False
        for device_id in range(8):  # For cuda:0 to cuda:7
            try:
                self.device = f'cuda:{device_id}'
